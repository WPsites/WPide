...
Plugin URI: 
Description: Replaces the default Wordpress editor with <a href="http://ace.ajax.org/">ACE</a>
Version: 0.5
Author: Simon Dunton
...
/*
take a look at http://wordpress.org/extend/plugins/advanced-code-editor/faq/
to get some extra code for ajax autosave and other stuff
*/
...
class AceEditor
{
	...
	function __construct() {
		...
		add_action('admin_head', 'AceEditor::add_admin_head');

		add_action('admin_head', 'AceEditor::load_editor');

		add_action('admin_init', 'AceEditor::add_admin_js');



		//setup ajax function to save a backup

		add_action('wp_ajax_ace_backup_call', 'AceEditor::ace_backup_call');
	...
	public static function ace_backup_call() {
		...
		if ($edit_type==='theme'){
			...
			$theme_root = get_theme_root();
				...
				$new_file_path_daily = WP_PLUGIN_DIR.'/ace/backups/themes'.$short_path.'.'.date("Ymd");

				$new_file_path_hourly = WP_PLUGIN_DIR.'/ace/backups/themes'.$short_path.'.'.date("YmdH");
				...
				$short_path = str_replace($plugin_root, '', $file_name);
                                ...
                                $new_file_path_daily = WP_PLUGIN_DIR.'/ace/backups/plugins/'.$short_path.'.'.date("Ymd");

                                $new_file_path_hourly = WP_PLUGIN_DIR.'/ace/backups/plugins/'.$short_path.'.'.date("YmdH");
		...
		//echo "final debug info : " . WP_PLUGIN_DIR.'/ace/backups/'.$short_path.'.backup';
...
if ( $_SERVER['PHP_SELF'] === '/wp-admin/plugin-editor.php' || 
	...
	add_action("init", create_function('', 'new AceEditor();'));
